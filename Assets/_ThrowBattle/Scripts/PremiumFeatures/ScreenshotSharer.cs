...
namespace _ThrowBattle
{
    public class ScreenshotSharer : MonoBehaviour
    {
        ...
        void GameManager_GameStateChanged(GameState newState, GameState oldState)
        {
            ...
            if (newState == GameState.Playing)
            {
                if (PremiumFeaturesManager.Instance.enablePremiumFeatures && !disableSharing && (sharedImageFormat == SharedImageType.GIF || sharedImageFormat == SharedImageType.Both))
                {
                    ...
                    recordCamera = Camera.main;
                    recorder = recordCamera.GetComponent<Recorder>();

                    if (recorder == null)
                    {
                        recorder = recordCamera.gameObject.AddComponent<Recorder>();
        ...
        IEnumerator CRCaptureScreenshot()
        {
            ...
            recordCamera.targetTexture = tempRT;
            recordCamera.Render();
            yield return null;
